...
namespace GDTextNg {
    ...
    class Color {
         ...
         */
        public function __construct(protected int $red = 0, protected int $green = 0, protected int $blue = 0, protected ?int $alpha = null) {}
        ...
        public static function fromHsl(float $h, float $s, float $l): self {
            ...
            return self::fromFloat($rgb);
        }

        private static function fromFloat(array $rgb): self {
            foreach ($rgb as &$v) {
                $v = round($v * 255);
            }
         ...
         *
         * @return int Returns the index of the specified color+alpha in the palette of the image, or index of allocated color if the color does not exist in the image's palette.
         */
        public function getIndex($image): int {
            $index = $this->hasAlphaChannel() ? imagecolorexactalpha($image, $this->red, $this->green, $this->blue, $this->alpha) : imagecolorexact($image, $this->red, $this->green, $this->blue);
            if ($index !== -1) {
                return $index;
            }
            return $this->hasAlphaChannel() ? imagecolorallocatealpha($image, $this->red, $this->green, $this->blue, $this->alpha) : imagecolorallocate($image, $this->red, $this->green, $this->blue);
        }
